...
// Global IoT Provider that maintains a single, persistent WebSocket MQTT connection
// across the entire application lifetime.
// Eliminates socket disconnects, loading screen flashes, and data refetches on page transitions.
...
export interface PowerData {
  ...
  power: number; // Watts
  energy: number; // kWh
  pf: number; // Power factor (0.00 - 1.00)
}
...
export interface RelayData {
  r1: boolean;
  ct1: boolean;
  t1: number; // Seconds remaining, 0 if inactive
  tAct1: "off" | "on" | "none";
  r2: boolean;
  ct2: boolean;
  t2: number;
...
export interface IoTContextType {
  data: IoTPayload | null;
  isConnecting: boolean;
  error: boolean;
  logs: LogEntry[];
  clearLogs: () => void;
  toggleRelay: (id: number) => void;
  pendingRelayIds: number[];
  wakeServer: () => void;
  setRelayTimer: (id: number, seconds: number, action?: "off" | "on") => void;
}
...
const TOPIC_RELAY_TIMER_ACK = "home/node1/relay/+/timer_ack";
...
export function IoTProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Connect directly to EMQX Cloud over WebSockets (TLS) — Single Global Connection
    const randomId = Math.random().toString(16).substring(2, 8);
    const client = mqtt.connect(MQTT_URL, {
      username: MQTT_USER,
      password: MQTT_PASS,
      clientId: `web-dash-global-${randomId}`,
      clean: true,
    ...
    client.on("connect", () => {
      console.log("[MQTT] Connected to EMQX Cloud broker (Global Session)");
      setError(false);
      client.subscribe(
        [TOPIC_TELEMETRY, TOPIC_STATUS, TOPIC_HEARTBEAT, TOPIC_RELAY_ACK, TOPIC_RELAY_TIMER_ACK, TOPIC_WOL_ACK],
        (err) => {
          if (err) {
        ...
        if (topic === TOPIC_TELEMETRY) {
          ...
          const transformed: IoTPayload = {
            ...
            power: {
              ...
              pf: typeof json.power?.pf === "string" ? parseFloat(json.power.pf) : (json.power?.pf ?? 1.0),
            },
            pir: {
...
export function useIoTContext(): IoTContextType {
