...
// Includes interactive radial dial, real-time "Feels Like" temperature, and ASHRAE mold spore germination risk.
...
import MoldComfortCard from "@/components/MoldComfortCard";
...
export default function ClimatePage() {
  const { data, isConnecting, toggleRelay } = useIoTData();
  ...
  const aqNum =
    typeof data.env.airQuality === "number"
      ? data.env.airQuality
      : parseInt(String(data.env.airQuality)) || 100;
          ...
          <AnimatePresence mode="wait">
            {activeTab === "temperature" ? (
              ...
              >
                ...
                {/* ── Mold & Comfort Analytics Preview ───────────────────────── */}
                <MoldComfortCard
                  temperature={data.env.roomTemp}
                  humidity={data.env.roomHumidity}
                  pressure={data.env.pressure ?? 1009.1}
              ...
              <motion.div
                ...
                className="space-y-4"
              >
                {/* ── Mold & Thermal Comfort Full Assessment ────────────────── */}
                <MoldComfortCard
                  temperature={data.env.roomTemp}
                  humidity={data.env.roomHumidity}
                  pressure={data.env.pressure ?? 1009.1}
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    ...
                    <div className="p-3 rounded-2xl bg-[#171E2C] border border-white/[0.05]">
                      ...
                      <span className="text-base font-bold text-white">
                        {(data.env.pressure ?? 1009.1).toFixed(1)}{" "}
                        <span className="text-xs font-normal text-neutral-400">hPa</span>
                      </span>
